...
class BaseRunner(object):
    ...
    def stop_container(self, container, cont_cmd=None, quiet=False,
        ...
        if cconfig and 'stop_grace_period' in cconfig.keys():
            ...
            config.update({'stop_grace_period': '10s'})
        builder = BaseBuilder(config_id=conf_id, config=config, runner=self,
                              labels=None)
        builder.string_arg(config, cmd, 'stop_signal', '--stop-signal')
        builder.string_arg(config, cmd,
